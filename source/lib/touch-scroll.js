		...
		init: function(options) {
			return this.each(function() {
				...
				var update = this.update = function() {
					height = $this.width();
					scrollHeight = o.scrollHeight || $this.attr('scrollWidth'); // jQuery 1.6 uses .prop(), older versions use .attr()
					
					if (scrollHeight < height) {
						scrollHeight = height;
					}
					maxHeight = $this.parent().width() - height;
          ...
          dispatchMouseEvent('mousedown', touch, getRootNode(touch.target));
				...
				function touchMove(e) {
					...
					var dy = getTouches(e)[0].pageX - touchY;
